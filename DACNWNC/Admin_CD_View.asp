...
<%
	...
	case "add_new"
		...
		if (not rs.eof) then
			...
			sql = "insert into ChuDe(TenChuDe,AnhPLM,MoTaChuDe) values ('" & adTenChuDe & "','" & adAnhPLM & "','" & adMoTaChuDe & "')"
			'Response.write(sql)
			conn.execute sql 
   ...
   Session("product_error")="Không thể xoá chủ đề vì còn tồn tại thông tin liên quan "
	...
	case "save_edit"
		...
		if (not rs.eof) then
			...
			sql = "update ChuDe set TenChuDe='" & adTenChuDe & "',AnhPLM='" & adAnhPLM& "',MoTaChuDe = '" & adMoTaChuDe & "' where IDChuDe = " & adIDChuDe
			'Response.write(sql)
			conn.execute sql 
