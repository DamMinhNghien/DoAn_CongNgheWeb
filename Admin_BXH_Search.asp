...
<body>
    ...
    <div id="navbar">
       <a href="Admin_CS_Search.asp" class="nav-link"onclick="openPage('CaSi')">Ca Sĩ</a>
        ...
        <a href="Admin_BXH_Search.asp" class="nav-link" onclick="openPage('BXH')" style="background-color: green; color: white;">BXH</a>
	...
	<body>
        ...
        <center><a href="Admin_BXH_View.asp?action=add">Thêm mới Top  </a></center>
		...
		<table align="center" border="1" width="100%">
			...
			%>
				<tr>
                    <td><%=rs("IDTop")%></td>
                    <td><%=rs("TenTop")%></td>
                    <td><img src="images/<%=rs("AnhTOP")%>" width=200></td>
                    <td><%=rs("MotaTop")%></td>
                    <td><%=rs("Loai")%></td>
                    <td><%=rs("TopCategories")%></td>
                    <td><a href="Admin_BXH_View.asp?action=edit&IDTop=<%=rs("IDTop")%>">Sửa</a></td>
                    <td><a onclick="return confirm('Bạn có chắc muốn xoá <%=rs("IDTop")%> hay ko?');"  href="Admin_BXH_View.asp?action=delete&IDTop=<%=rs("IDTop")%>">Xóa</a></td>
                </tr>
			<%
				rs.movenext
				Wend
