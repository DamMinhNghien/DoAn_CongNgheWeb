...
Set rs2 = Server.CreateObject("ADODB.Recordset")
            sql2="select * from CaSi where BiDanh = '" & SingerID & "'"
    rs2.open sql2, conn
Set rs1 = Server.CreateObject("ADODB.Recordset")
sql1 = "SELECT BaiHat.IDBaiHat, BaiHat.TenBaiHat, BaiHat.NgayPhatHanh, BaiHat.QuocGia,BaiHat.AnhBH, ChuDe.TenChuDe, ChuDe.MoTaChuDe, ChuDe.AnhPLM " & _
       "FROM BaiHat " & _
       "JOIN CaSi ON BaiHat.BiDanh = CaSi.BiDanh " & _
       "JOIN ChuDe ON BaiHat.IDChuDe = ChuDe.IDChuDe " & _
       "WHERE CaSi.BiDanh LIKE '" & SingerID & "'"
...
%>
            ...
            <div class="playlist-content">
               <div class="playlist-cover">
                  <img src="images\<%=rs2("AnhCS")%>" alt="">
               </div>
               <div class="playlist-info">
                  <div class="playlist-public"><%=rs2("TenCaSi")%></div>
                  <div class="playlist-title"><%=rs2("BiDanh")%></div>
    ...
    <div class="App__section-grid-container">
      <%Set rs3 = Server.CreateObject("ADODB.Recordset")
            sql3="SELECT pcn.IDPlayList,pcn.AnhPL,pcn.TenPlayList " & _
           "  FROM BaiHat AS b " & _
           " JOIN BaiHatPlayListCN AS bp ON b.IDBaiHat = bp.IDBaiHat " & _
           " JOIN PlayListCaNhan AS pcn ON bp.IDPlayList = pcn.IDPlayList" &_
           " Where BiDanh = '" & SingerID & "' group by pcn.IDPlayList,pcn.AnhPL,pcn.TenPlayList"
    rs3.open sql3, conn
        If rs3.eof then%>
         <h1 style="color: white;text-align: center;"></h1>
         <%else
         while not rs3.eof    
         idalbum=rs3("IDPlayList")%>
                    <div class="App__section-grid-item" onclick="redirectFunction4('<%=idalbum%>')">
                        <div class="image-container">
                            <img class="round-border-image" src="images\<%=rs3("AnhPL")%>" style="width:178px; height: 180px;">
                        ...
                        <h3 style="text-align:center;"><%=rs3("TenPlayList")%></h3>
                    </div>
    <%
    rs3.movenext
    wend
